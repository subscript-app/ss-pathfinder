...
use crate::gpu::d3d11::shaders::{ProgramsD3D11, PROPAGATE_WORKGROUP_SIZE, SORT_WORKGROUP_SIZE};
use crate::gpu::d3d11::shaders::{BIN_WORKGROUP_SIZE, BOUND_WORKGROUP_SIZE, DICE_WORKGROUP_SIZE};
...
use ss_pathfinder_gpu::allocator::{BufferTag, GPUMemoryAllocator, GeneralBufferID};
...
pub(crate) struct RendererD3D11<D>
where
    D: Device,
{
    ...
    D: Device,
{
    ...
    fn bound(
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                uniforms: &[
                    (
                        &bound_program.path_count_uniform,
                        UniformData::Int(tile_path_info.len() as i32),
                    ),
                    (
                        &bound_program.tile_count_uniform,
                        UniformData::Int(tile_count as i32),
                    ),
                ],
                images: &[],
                storage_buffers: &[
                    (
                        &bound_program.tile_path_info_storage_buffer,
                        tile_path_info_buffer,
                    ),
    ...
    fn bin_segments(
        ...
        core.device.upload_to_buffer::<u32>(
            &z_buffer,
            0,
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                storage_buffers: &[
                    ...
                    (
                        &bin_program.metadata_storage_buffer,
                        propagate_metadata_buffer,
                    ),
        ...
        core.current_timer
            .as_mut()
            .unwrap()
            .push_query(TimeCategory::Bin, timer_query);

        let indirect_draw_params_receiver =
            core.device
                .read_buffer(z_buffer, BufferTarget::Storage, 0..32);
    ...
    fn dice_segments(
        ...
        let dice_indirect_draw_params_buffer = core
            .allocator
            .get_general_buffer(dice_indirect_draw_params_buffer_id);
        ...
        let points_buffer = core
            .allocator
            .get_general_buffer(points_buffer_id.expect("Where's the points buffer?"));
        ...
        let workgroup_count = (batch_segment_count + DICE_WORKGROUP_SIZE - 1) / DICE_WORKGROUP_SIZE;
        let compute_dimensions = ComputeDimensions {
            x: workgroup_count,
            y: 1,
            z: 1,
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                uniforms: &[
                    (
                        &dice_program.transform_uniform,
                        UniformData::Mat2(transform.matrix.0),
                    ),
                    (
                        &dice_program.translation_uniform,
                ...
                storage_buffers: &[
                    ...
                    (
                        &dice_program.input_indices_storage_buffer,
                        point_indices_buffer,
                    ),
                    (&dice_program.microlines_storage_buffer, microlines_buffer),
                    (
                        &dice_program.dice_metadata_storage_buffer,
                        &dice_metadata_storage_buffer,
                    ),
                ],
            },
        ...
        core.current_timer
            .as_mut()
            .unwrap()
            .push_query(TimeCategory::Dice, timer_query);
        ...
        core.allocator
            .free_general_buffer(dice_indirect_draw_params_buffer_id);
        ...
        Some(MicrolinesBufferIDsD3D11 {
            buffer_id: microlines_buffer_id,
    ...
    fn draw_fills(
        ...
        let &PropagateTilesInfoD3D11 {
            ref alpha_tile_range,
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                storage_buffers: &[
                    ...
                    (
                        &fill_program.alpha_tiles_storage_buffer,
                        &alpha_tiles_buffer,
                    ),
                ],
            },
    ...
    fn propagate_tiles(
        ...
        core.device
            .upload_to_buffer(z_buffer, 0, &vec![0i32; tile_area], BufferTarget::Storage);
        ...
        let mut storage_buffers = vec![
            (
                &propagate_program.draw_metadata_storage_buffer,
                propagate_metadata_storage_buffer,
            ),
            (
                &propagate_program.backdrops_storage_buffer,
                &backdrops_storage_buffer,
            ),
            (
                &propagate_program.draw_tiles_storage_buffer,
                tiles_d3d11_buffer,
            ),
            (&propagate_program.z_buffer_storage_buffer, z_buffer),
            (
                &propagate_program.first_tile_map_storage_buffer,
                first_tile_map_storage_buffer,
            ),
            (
                &propagate_program.alpha_tiles_storage_buffer,
        ...
        match clip_buffer_ids {
            Some(clip_buffer_ids) => {
                ...
                let clip_metadata_buffer =
                    core.allocator.get_general_buffer(clip_metadata_buffer_id);
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                uniforms: &[
                    ...
                    (
                        &propagate_program.column_count_uniform,
                        UniformData::Int(column_count as i32),
                    ),
                    (
        ...
        let fill_indirect_draw_params_receiver =
            core.device
                .read_buffer(&z_buffer, BufferTarget::Storage, 0..32);
        let fill_indirect_draw_params =
            core.device.recv_buffer(&fill_indirect_draw_params_receiver);
    ...
    fn sort_tiles(
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                images: &[],
                uniforms: &[(
                    &sort_program.tile_count_uniform,
                    UniformData::Int(tile_count),
                )],
                storage_buffers: &[
                    (&sort_program.tiles_storage_buffer, tiles_d3d11_buffer),
                    (
                        &sort_program.first_tile_map_storage_buffer,
                        first_tile_map_buffer,
                    ),
    ...
    pub(crate) fn draw_tiles(
        ...
        match clear_color {
            None => {
                ...
                uniforms.push((
                    &tile_program.clear_color_uniform,
        ...
        core.device.dispatch_compute(
            ...
            &ComputeState {
                ...
                storage_buffers: &[
                    ...
                    (
                        &tile_program.first_tile_map_storage_buffer,
                        first_tile_map_storage_buffer,
                    ),
                ],
                uniforms: &uniforms,
